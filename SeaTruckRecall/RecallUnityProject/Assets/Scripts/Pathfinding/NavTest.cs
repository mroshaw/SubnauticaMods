...
public class NavTest : MonoBehaviour
{
    ...
    public int numCellExtends = 100;

    public void Test()
    {
        NavCell[,,] grid = GenerateNavGrid(source, target, cellSize, numCellExtends);
        List<NavCell> path = FindPath(grid, source.position, target.position);
        ...
        {
            Debug.Log($"Path found in {path.Count} cells");
            foreach (NavCell cell in path)
            {
                GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                sphere.transform.position = cell.Position;
    ...
    public static NavCell[,,] GenerateNavGrid(Transform sourceTransform, Transform targetTransform, float cellSize, int numCellExtends)
    {
